...
function createRoutes({ collector, monitorControl, db, onEvent, features, reloadFeatures, reloadAlarms, patchAlarmRule }){
	...
	router.get('/api/storage', (req, res)=>{
		...
		// drop rows no live target can produce any more, so a removed target (or a
		// switched-off mount layer) stops reporting its last scan as if it were
		// current. With no targets at all the scope is pure history: show it as-is.
		const liveKinds = new Set(targets.flatMap((t)=>t.kinds || []));
		const keep = (kind)=>!targets.length || liveKinds.has(kind);
		const byKind = {};
		for(const e of entries){ if(keep(e.kind)) (byKind[e.kind] = byKind[e.kind] || []).push(e); }
