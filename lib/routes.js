...
function createRoutes({ collector, monitorControl, db, onEvent, serviceChecker, storage }){
	...
	router.get('/api/storage/scopes', (req, res)=>{
		...
		const names = new Set([...Object.keys(online), ...(storage ? storage.scopes() : []), ...db.distinctStorageScopes()]);
		res.json({ scopes: [...names].sort().map((name)=>({ name, online: !!online[name] })) });
	});
	...
	router.get('/api/storage', (req, res)=>{
		...
		res.json({
			...
			mounts,
			kinds: byKind,
			targets: storage ? storage.status(scope) : [],
		});
	});
	...
	router.post('/api/storage/scan', async (req, res)=>{
		...
		const ran = storage ? await storage.scanNow(scope) : false;
		res.json({ ok: true, ran, targets: storage ? storage.status(scope) : [] });
	});

	// flat list of configured services with their current status (independent of servers)
	router.get('/api/services', (req, res)=>{
		res.json({ services: serviceChecker ? serviceChecker.all() : [] });
	});
	...
	router.post('/api/services/check', async (req, res)=>{
		try{ if(serviceChecker) await serviceChecker.checkNow(); }
