...
function createRoutes({ collector, monitorControl, db, onEvent, serviceChecker }){
	...
	// ?server=<name> -> live per-mount capacity + latest per-account scan
	router.get('/api/storage', (req, res)=>{
		const server = req.query.server;
		if(!server) return res.status(400).json({ error: 'server parameter required' });
		const live = collector.states[server];
		...
		const mounts = (liveDisks && liveDisks.length)
			...
			: db.latestDisks(server);
		const usage = db.latestStorageUsage(server);
		res.json({
			server,
			online: !!(live && live.online),
			scanned_at: usage.ts,
			total_scanned: usage.rows.reduce((s, r)=>s + (r.bytes || 0), 0),
			accounts: usage.rows,
			mounts,
		});
	});
