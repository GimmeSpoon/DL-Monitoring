...
function createMock(collector, { onEvent } = {}){
	...
	for(const s of MOCK_SERVERS){
		sim[s.name] = {
			...
			net: { rx: 0, tx: 0 },          // cumulative bytes; collector derives the rate
			offlineUntil: 0,
		};
	}
	...
	function tick(){
		for(const server of MOCK_SERVERS){
			...
			for(let i = 0; i < server.gpus; i++){
				...
				const gpuJobs = st.jobs.filter((j)=>j.gpu_index === i);
				const used = gpuJobs.reduce((a, j)=>a + j.used_memory, 0);
				gpus.push({
					'procs': gpuJobs.length,
					'processes': gpuJobs.map((j)=>({ pid: j.pid, user: j.user, used_memory: j.used_memory })),
					'gpu_id': String(i),
					'timestamp': timestamp(),
			...
			// network scales with GPU load: busier boxes shuffle more data
			st.net.rx += Math.round((20 + st.util * 12 + Math.random() * 40) * 1e6);
			st.net.tx += Math.round((10 + st.util * 5 + Math.random() * 20) * 1e6);
			...
			collector.ingest(server.name, {
				...
				'sysRaw': {
					...
					'network': { 'rx': st.net.rx, 'tx': st.net.tx },
				},
			});
		}
	}
