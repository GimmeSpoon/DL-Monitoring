...
// "<bytes>\t<name>" (du) -> {name, bytes}. Blank/garbage lines are dropped.
function parseSized(lines){
	const out = [];
	for(const line of lines || []){
		const m = String(line).match(/^\s*(\d+)\s+(.+?)\s*$/);
		if(m) out.push({ name: m[2], bytes: Number(m[1]) });
	}
	return out;
}
...
function accountsCommand({ roots, sudo, strategy }){
	...
	const du = `echo @@DU; ${S}du -sb ${globs} 2>/dev/null`;
...
function mountsCommand({ sources, sudo }){
	return `${sudo ? 'sudo -n ' : ''}du -sb ${sources.map(sq).join(' ')} 2>/dev/null`;
}
	...
	async function runContainers(t){
		...
		if(t.layers.includes('mounts')){
			const sources = [...new Set(containers.flatMap((c)=>c.mounts.map((m)=>m.source)))]
				.filter((s)=>s.startsWith('/') && !t.excludeMounts.some((p)=>s === p || s.startsWith(p.replace(/\/+$/, '') + '/')));
			if(sources.length){
		...
		return { entries: buildContainerEntries(containers, sizeBySource), method: t.layers.join('+') };
	}
...
module.exports = {
	createStorageScanner, normalize,
