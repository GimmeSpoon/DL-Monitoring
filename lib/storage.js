...
// Which entry kinds a target can still produce. Rows of any other kind are
// left over from a target that was removed or had its mount layer switched
// off; without this the API keeps serving their last good numbers forever.
function targetKinds(t){
	if(t.type === 'accounts') return ['account'];
	if(t.type === 'paths') return ['path'];
	if(t.type === 'command') return ['custom'];
	if(t.type === 'containers') return t.layers.includes('mounts') ? ['container', 'mount'] : ['container'];
	return [];
}
...
function createStorageScanner({ targets, db, onEvent, intervalMs, timeoutMs, firstDelayMs }){
	...
	async function scanTarget(t){
		...
		for(const e of entries){
			db.insertStorageEntry(ts, t.scope, e.kind, e.parent || '', e.name, e.bytes, e.meta ? JSON.stringify(e.meta) : null, t.id);
		}
	...
	return {
		...
		status: (scope)=>list.filter((t)=>!scope || t.scope === scope).map((t)=>({
			id: t.id, scope: t.scope, type: t.type, label: t.label, connection: t.connection, kinds: targetKinds(t),
...
module.exports = {
	createStorageScanner, normalize, makeMatcher, targetKinds,
