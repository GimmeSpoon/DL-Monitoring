...
function openDb(dataDir, filename = 'monitor.db'){
	...
	const stmt = {
		...
		storage: db.prepare('INSERT OR REPLACE INTO storage_usage (ts,server,account,bytes) VALUES (?,?,?,?)'),
	};
	...
	return {
		...
		insertStorageUsage: (ts, server, account, bytes)=>stmt.storage.run(ts, server, account, bytes),

		// most recent scan's per-account rows for a server (largest first)
		latestStorageUsage: (server)=>{
			const row = db.prepare('SELECT MAX(ts) AS ts FROM storage_usage WHERE server=?').get(server);
			if(!row || row.ts == null) return { ts: null, rows: [] };
			const rows = db.prepare('SELECT account, bytes FROM storage_usage WHERE server=? AND ts=? ORDER BY bytes DESC').all(server, row.ts);
			return { ts: row.ts, rows };
		},
		...
		prune: ({ metricsDays, eventsDays, usageDays, storageDays })=>{
			...
			db.prepare('DELETE FROM storage_usage WHERE ts < ?').run(now - storageDays * 86400);
		},
