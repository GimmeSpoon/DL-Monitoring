...
CREATE TABLE IF NOT EXISTS storage_entries (
	...
	bytes INTEGER, meta TEXT, target TEXT NOT NULL DEFAULT '',
...
function migrate(db){
	for(const sql of [
		...
		// which target produced a row. Without it two targets of the same kind in
		// one scope (say two `command` targets) share a MAX(ts) and the later scan
		// hides the earlier one's rows entirely.
		"ALTER TABLE storage_entries ADD COLUMN target TEXT NOT NULL DEFAULT ''",
	]){
		try{ db.exec(sql); }
...
function openDb(dataDir, filename = 'monitor.db'){
	...
	const stmt = {
		...
		storage: db.prepare('INSERT OR REPLACE INTO storage_entries (ts,scope,kind,parent,name,bytes,meta,target) VALUES (?,?,?,?,?,?,?,?)'),
	};
	...
	return {
		...
		insertStorageEntry: (ts, scope, kind, parent, name, bytes, meta, target)=>stmt.storage.run(ts, scope, kind, parent, name, bytes, meta, target || ''),

		// Latest scan per *target*, not per scope: targets run on independent
		// schedules, so a scope-wide MAX(ts) would hide every one that simply
		// hasn't been due as recently as another. Grouping by kind alone is not
		// enough either — two targets of the same kind in one scope (two `command`
		// targets, say) would then take turns hiding each other's rows.
		latestStorageEntries: (scope)=>{
			...
			const rows = db.prepare(`
				SELECT e.ts, e.kind, e.parent, e.name, CAST(e.bytes AS REAL) AS bytes, e.meta, e.target
				FROM storage_entries e
				JOIN (SELECT kind, target, MAX(ts) AS ts FROM storage_entries WHERE scope=? GROUP BY kind, target) m
				  ON e.kind = m.kind AND e.target = m.target AND e.ts = m.ts
