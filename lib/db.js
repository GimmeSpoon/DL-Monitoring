...
function migrate(db){
	...
	// any (kind, name) pair. Carry the old history over once so an upgrade doesn't
	// look like the storage scans lost 90 days.
	const empty = db.prepare('SELECT COUNT(*) AS c FROM storage_entries').get().c === 0;
	if(empty){
	...
	return {
		...
		latestStorageEntries: (scope)=>{
			const rows = db.prepare(`
				SELECT e.ts, e.kind, e.parent, e.name, e.bytes, e.meta
		...
		prune: ({ metricsDays, eventsDays, usageDays, storageDays })=>{
			...
			db.prepare('DELETE FROM storage_usage WHERE ts < ?').run(now - storageDays * 86400); // legacy table, no longer written
		},
