...
function parse(servn, raw){
	...
	// The process list rides along for the dashboard's hover popover.
	const procsByGpu = {};
	for(const a of apps){
		if(a.gpu_index !== null && a.gpu_index !== undefined){
			(procsByGpu[a.gpu_index] = procsByGpu[a.gpu_index] || []).push({ pid: a.pid, user: a.user, used_memory: a.used_memory });
		}
	}
	for(const g of gpus){
		const list = procsByGpu[Number(g.gpu_id)] || [];
		g.procs = list.length;
		g.processes = list;
	}
	...
	// NET: /proc/net/dev — cumulative rx/tx bytes, summed over the physical
	// interfaces (loopback and virtual/container ifaces are skipped so bridged
	// traffic isn't double-counted). The collector turns these into a rate.
	let network = null;
	const IGNORE_IFACE = /^(lo|veth|docker|br-|virbr|vnet|tap|cni|flannel|kube)/;
	for(const line of sec.NET || []){
		const m = line.match(/^\s*([\w.@-]+):\s*(.*)$/);
		if(!m || IGNORE_IFACE.test(m[1])) continue;
		const cols = m[2].trim().split(/\s+/).map(Number);
		if(cols.length < 9 || !Number.isFinite(cols[0]) || !Number.isFinite(cols[8])) continue;
		if(network === null) network = { rx: 0, tx: 0 };
		network.rx += cols[0]; // receive bytes
		network.tx += cols[8]; // transmit bytes
	}
	...
	return {
		...
		'gpus': gpus,
		'apps': apps,
		'sysRaw': { cpu, memory, disks, network },
	};
}
